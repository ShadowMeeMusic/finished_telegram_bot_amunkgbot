...
from database import init_db, get_or_create_user, enable_wal, get_bot_status
...
@dp.message(F.text == "📊 Статистика")
async def text_stats_admin(message: types.Message):
    from handlers.admin import stats
    await stats(message)
...
# Новый middleware: приостановка бота
@dp.update.middleware()
async def pause_middleware(handler, event, data):
    if hasattr(event, "from_user") and event.from_user:
        user_id = event.from_user.id
        # Разрешаем админам и Глав Тех
        if user_id in CHIEF_ADMIN_IDS or user_id == TECH_SPECIALIST_ID:
            return await handler(event, data)

        status = await get_bot_status()
        if status.is_paused:
            await event.answer(
                f"🛑 Бот временно приостановлен.\n"
                f"Причина: {status.pause_reason or 'Технические работы'}\n"
                "Попробуйте позже."
            )
            return
